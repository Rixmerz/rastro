...
export class Recorder {
  ...
  async pollDomActivity(page: Page): Promise<void> {
    ...
    if (state.added !== previous.added || state.removed !== previous.removed || state.attributes !== previous.attributes) {
      this.markActivity();
    }
  ...
  private isPendingCandidate(resourceType: string, hasInterval: boolean): boolean {
    return !NON_BLOCKING_RESOURCE_TYPES.has(resourceType) && !hasInterval;
  }
  ...
  private onRequestWillBeSent(e: CdpRequestWillBeSent, tabId: string): void {
    ...
    if (this.isPendingCandidate(resourceType, hasInterval)) {
  ...
  private onResponseReceived(e: CdpResponseReceived, tabId: string): void {
    ...
    if (this.inFlight.delete(e.requestId)) this.markActivity();
  }
